...
- **Starting against a password-less Redis prints an unmissable warning** — session tokens are stored in Redis, so an unprotected instance lets anyone who can read it become an authenticated admin without touching the login page. Starting is still allowed; the banner goes to stderr rather than the logger, because a warning that `log_level` can silence is not a warning
