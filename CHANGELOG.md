...
### Added
- **CI refuses a release whose CLI would ship an outdated TUI** — the root module pins `tui` at an exact version, and nothing enforced that it was the newest `tui` tag. That link rotted for six months and shipped a February terminal UI to everyone who installed the CLI (fixed in 0.7.0), and nothing prevented it recurring: `go.work` substitutes the local `tui` directory, so every local build, test and manual run reads the current sources while only consumers resolving from the module proxy get the stale one. A dedicated job now compares the pinned version against the highest `tui/vX.Y.Z` tag and fails when the pin is behind, naming both versions and the command that fixes it. Unreleased work on `tui` cannot trip it — it compares versions, never the source tree. `scripts/release-check.sh` covers the half a version comparison cannot see, reporting when `tui/` sources have moved past their newest tag, which is the same stale-TUI outcome reached by nobody tagging `tui` at all
...
### Changed
- **Minimum Go version is now 1.25** — both modules declare `go 1.25.0`. The directive said 1.26 only because that was the toolchain in use, not because anything in the tree needed it, which shut out every Go 1.25 user for no reason. 1.25.0 is the real floor: `golang.org/x/crypto` (together with `x/term`, `x/sys`, and `x/text`) declares it, and no toolchain below that can build the tree. No dependency version changed other than `tui`, which the root now requires at v0.3.1

### Fixed
- **The CLI shipped a six-month-old TUI** — the root module required `tui v0.1.0` while the `tui` module had long since moved to v0.3.x, so `go install github.com/benedict-erwin/gqm/cmd/gqm@v0.6.0` built a terminal UI frozen in February. Installing the CLI now gets the current TUI: the DAG tab, the drill-down views, the 0.4.0 visual restyle, and the stale-job markers added in 0.5.0 — none of which the pinned version had. No developer saw the old build, because `go.work` substitutes the local `tui` directory and every local build, test, and manual run therefore used the current sources; only users installing from the proxy got the stale one
- **Documented Go version** — the README advertised Go 1.22+, a minimum no build of this module would accept: `golang.org/x/crypto` alone already rules out anything below 1.25. Badge, requirements, and Built With now match the `go` directive. The Redis requirement no longer cites `BLMOVE`, which the dequeue path stopped using when it moved to `RPOP` plus a Lua script

## [0.6.0] — 2026-08-09

A consistency release: dead-letter counts and dead-letter listings now
always agree. A retention-expired job could leave a phantom member in a
queue's dead-letter set forever, so dashboards counted a dead job that no
listing could show. Those members are now cleaned up automatically — which
is also why this is a minor bump rather than a patch: counters visibly
drop to the truth, and anything reading the raw sorted set inherits the
correction. The public Go API is unchanged; the `tui` module is untouched
and gets no new tag.
...
died mid-job stranded its in-flight jobs in `processing` forever, with no
recovery path. Those jobs are now reclaimed automatically, can be canceled
manually, and are flagged in every UI while they wait.

Operators upgrading should expect one-time movement: jobs stuck in
`processing` from past crashes will be retried or dead-lettered shortly
after the first upgraded instance starts. The public Go API is unchanged
(`CancelJob` now accepts a status it previously rejected); API responses
gain additive fields only. The `tui` module is tagged `tui/v0.3.0`.
...
### Changed
- Embedded dashboard `VERSION` bumped to 0.3.0 (stale badge in job lists and detail, next-step hint on stale job detail)
